...
export default function CommandCenterPage() {
  ...
  const [cashMgmtSelectedDate, setCashMgmtSelectedDate] = useState<'today' | 'tomorrow' | 'day2'>('today');
  const [nmOpsModal, setNmOpsModal] = useState<{ nm: string; opType: 'ikmal' | 'toplama'; dayKey: 'today' | 'tomorrow' | 'day2'; operatorName: string } | null>(null);
...
Otomatik Eskalasyon Sistemi
  ...
  // ─────────────────────────────────────────────────────────
  // 🧠 AI BRAIN: Tüm ATM verisi analiz edilir, kararlar üretilir
  // Bu hesaplama tüm kartları, listeleri ve operasyon planını besler
  // ─────────────────────────────────────────────────────────
  const aiBrainData = useMemo(() => {
    const allAtms = atmMasterData as any[];

    const processedAtms = allAtms.map((atm: any) => {
      const idSeed = String(atm.atm_id).split('').reduce((a: number, c: string) => a + c.charCodeAt(0), 0);
      const zone = parseInt(atm.zone || '3');
      const isSalary = atm.salary_flag === 'Maaş Ödemeli';

      // Deterministik nakit seviyesi (5–95%)
      const cashLevel = 5 + ((idSeed * 17 + zone * 13) % 91);

      // Aciliyet skoru
      let urgency = cashLevel < 15 ? 100 : cashLevel < 25 ? 80 : cashLevel < 40 ? 55 : cashLevel > 85 ? 45 : 15;
      if (isSalary) urgency = Math.min(100, urgency + 20);
      if (zone === 1) urgency = Math.min(100, urgency + 15);

      // Karar
      let decision: 'ikmal' | 'toplama' | 'izle';
      let priority: 'CRITICAL' | 'HIGH' | 'MEDIUM' | 'LOW';
      if (cashLevel < 15)       { decision = 'ikmal';   priority = 'CRITICAL'; }
      else if (cashLevel < 30)  { decision = 'ikmal';   priority = 'HIGH'; }
      else if (cashLevel < 45)  { decision = 'ikmal';   priority = 'MEDIUM'; }
      else if (cashLevel > 82)  { decision = 'toplama'; priority = 'MEDIUM'; }
      else                      { decision = 'izle';    priority = 'LOW'; }

      // Tutar tahmini
      const capacityK = zone === 1 ? 1400 : zone === 2 ? 1100 : zone === 3 ? 900 : 700;
      const ikmalAmount = decision === 'ikmal'   ? Math.round((0.78 - cashLevel / 100) * capacityK) * 1000 : 0;
      const topAmount   = decision === 'toplama' ? Math.round((cashLevel / 100 - 0.28) * capacityK) * 1000 : 0;

      return { ...atm, cashLevel, urgency, decision, priority, ikmalAmount, topAmount };
    });

    // NM bazlı grupla
    const nmStats: Record<string, { ikmal: any[]; toplama: any[]; critical: number; total: number }> = {};
    processedAtms.forEach((atm: any) => {
      const nm = atm.cash_center || 'UNKNOWN';
      if (!nmStats[nm]) nmStats[nm] = { ikmal: [], toplama: [], critical: 0, total: 0 };
      nmStats[nm].total++;
      if (atm.decision === 'ikmal')   nmStats[nm].ikmal.push(atm);
      if (atm.decision === 'toplama') nmStats[nm].toplama.push(atm);
      if (atm.priority === 'CRITICAL') nmStats[nm].critical++;
    });
    // Urgency'ye göre sırala
    Object.values(nmStats).forEach(nm => {
      nm.ikmal.sort((a: any, b: any) => b.urgency - a.urgency);
      nm.toplama.sort((a: any, b: any) => b.urgency - a.urgency);
    });

    const globalCritical  = processedAtms.filter((a: any) => a.priority === 'CRITICAL').length;
    const globalIkmal     = processedAtms.filter((a: any) => a.decision === 'ikmal').length;
    const globalToplama   = processedAtms.filter((a: any) => a.decision === 'toplama').length;
    const totalIkmalAmt   = processedAtms.reduce((s: number, a: any) => s + a.ikmalAmount, 0);
    const totalTopAmt     = processedAtms.reduce((s: number, a: any) => s + a.topAmount, 0);

    return {
      processedAtms, nmStats,
      globalCritical, globalIkmal, globalToplama,
      totalIkmalAmt, totalTopAmt,
      totalAtms: processedAtms.length,
      aiSavings: 83.8,
      aiConfidence: 94,
      decisions: 5,
    };
  ...
  // Brain-driven NM ops (gün bazlı öncelik sırasına göre dilimle)
  const getBrainNmOps = (nmCenter: string, dayKey: 'today' | 'tomorrow' | 'day2') => {
    const nmKey = nmCenter.replace(' NM', '').trim();
    const nmData = aiBrainData.nmStats[nmKey];
    if (!nmData) return { ikmalCount: 0, topCount: 0, ikmalAmt: 0, topAmt: 0, ikmalAtms: [] as any[], topAtms: [] as any[] };
    const d = dayKey === 'today' ? 0 : dayKey === 'tomorrow' ? 1 : 2;
    const iPS = Math.max(2, Math.ceil(nmData.ikmal.length / 3));
    const tPS = Math.max(1, Math.ceil(nmData.toplama.length / 3));
    const ikmalAtms = nmData.ikmal.slice(d * iPS, (d + 1) * iPS);
    const topAtms   = nmData.toplama.slice(d * tPS, (d + 1) * tPS);
    return {
      ikmalCount: ikmalAtms.length,
      topCount:   topAtms.length,
      ikmalAmt:   ikmalAtms.reduce((s: number, a: any) => s + a.ikmalAmount, 0),
      topAmt:     topAtms.reduce((s: number, a: any) => s + a.topAmount, 0),
      ikmalAtms,
      topAtms,
    };
  };

  // Brain-driven ATM listesi (modal için)
  const getBrainNmAtmList = (nmCenter: string, opType: 'ikmal' | 'toplama', dayKey: 'today' | 'tomorrow' | 'day2') => {
    const ops = getBrainNmOps(nmCenter, dayKey);
    const src = opType === 'ikmal' ? ops.ikmalAtms : ops.topAtms;
    return src.map((atm: any) => ({
      atm_id: String(atm.atm_id),
      atm_name: atm.atm_name || `ATM ${atm.atm_id}`,
      city: atm.city,
      district: atm.district,
      location_type: atm.location_type,
      amount: opType === 'ikmal' ? atm.ikmalAmount : atm.topAmount,
      cashLevel: atm.cashLevel,
      priority: atm.priority,
      urgency: atm.urgency,
      isSalary: atm.salary_flag === 'Maaş Ödemeli',
      zone: atm.zone,
    }));
  };

  // Geriye dönük uyumluluk — eski getNmOps/getNmAtmList çağrılarını yönlendir
  const getNmOps = (nm: string, dayKey: 'today' | 'tomorrow' | 'day2') => getBrainNmOps(nm, dayKey);
  const getNmAtmList = (nmCenter: string, opType: 'ikmal' | 'toplama', dayKey: 'today' | 'tomorrow' | 'day2') => getBrainNmAtmList(nmCenter, opType, dayKey);
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-[#112544] rounded-2xl p-6 ring-1 ring-[#2B416B] transition-all duration-300 hover:ring-[#2E86FF]/50 hover:shadow-xl hover:shadow-[#2E86FF]/10">
        ...
        <div className="mb-4 bg-gradient-to-r from-[#0A1628] to-[#0E2142] rounded-xl p-3 ring-1 ring-[#2E86FF]/40">
          <div className="flex items-center justify-between flex-wrap gap-2">
            <div className="flex items-center gap-3">
              <div className="w-8 h-8 rounded-full bg-[#2E86FF]/20 ring-1 ring-[#2E86FF]/60 flex items-center justify-center text-base animate-pulse">
                🧠
              </div>
              <div>
                <div className="text-xs font-bold text-white">AI Karar Motoru Aktif</div>
                <div className="text-[10px] text-[#A7B8D8]">{aiBrainData.totalAtms.toLocaleString('tr-TR')} ATM analiz edildi • Güven: %{aiBrainData.aiConfidence} • Tasarruf: %{aiBrainData.aiSavings}</div>
              </div>
            </div>
            <div className="flex items-center gap-2 flex-wrap">
              <div className="px-2 py-1 rounded-lg bg-[#EF4444]/15 ring-1 ring-[#EF4444]/40 text-center">
                <div className="text-[10px] text-[#EF4444]">KRİTİK</div>
                <div className="text-sm font-bold text-white">{aiBrainData.globalCritical}</div>
              </div>
              <div className="px-2 py-1 rounded-lg bg-[#10B981]/15 ring-1 ring-[#10B981]/40 text-center">
                <div className="text-[10px] text-[#10B981]">İkmal</div>
                <div className="text-sm font-bold text-white">{aiBrainData.globalIkmal}</div>
              </div>
              <div className="px-2 py-1 rounded-lg bg-[#F2B705]/15 ring-1 ring-[#F2B705]/40 text-center">
                <div className="text-[10px] text-[#F2B705]">Toplama</div>
                <div className="text-sm font-bold text-white">{aiBrainData.globalToplama}</div>
              </div>
              <div className="px-2 py-1 rounded-lg bg-[#8B5CF6]/15 ring-1 ring-[#8B5CF6]/40 text-center">
                <div className="text-[10px] text-[#8B5CF6]">İkmal Toplam</div>
                <div className="text-sm font-bold text-white">₺{(aiBrainData.totalIkmalAmt / 1_000_000).toFixed(0)}M</div>
              </div>
            </div>
          </div>
        </div>
        ...
        {/* Tarih Seçici */}
        <div className="flex items-center gap-2 mb-4 p-3 bg-[#0E2142]/60 rounded-xl ring-1 ring-[#2B416B]">
          <span className="text-xs text-[#A7B8D8] font-semibold mr-1">📅 Operasyon Günü:</span>
          {(['today', 'tomorrow', 'day2'] as const).map((day) => {
            const labels: Record<string, string> = { today: 'Bugün', tomorrow: 'Yarın', day2: '+2 Gün' };
            const icons: Record<string, string> = { today: '🔴', tomorrow: '🟡', day2: '🔵' };
            const isActive = cashMgmtSelectedDate === day;
            return (
              <button
                key={day}
                onClick={() => setCashMgmtSelectedDate(day)}
                className={`px-4 py-1.5 rounded-lg text-xs font-semibold transition-all ${
                  isActive
                    ? 'bg-[#2E86FF] text-white ring-1 ring-[#2E86FF]'
                    : 'bg-[#112544] text-[#A7B8D8] hover:text-white hover:bg-[#1C2E52] ring-1 ring-[#2B416B]'
                }`}
              >
                {icons[day]} {labels[day]}
              </button>
            );
          })}
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
          {operators.map((operator) => {
            ...
            return (
              ...
              {/* NM Bazlı Operasyon Planı */}
              {operator.nmCenters.length > 0 && (
                <div className="bg-[#0A1628] rounded-lg p-2 mb-2 relative z-10 ring-1 ring-[#2B416B]/60">
                  <div className="text-xs text-[#A7B8D8] font-semibold mb-2">📋 NM Bazlı Operasyon Planı</div>
                  <div className="space-y-2">
                    {operator.nmCenters.map((nm) => {
                      const ops = getNmOps(nm, cashMgmtSelectedDate);
                      const totalAmt = ops.ikmalAmt + ops.topAmt;
                      return (
                        <div key={nm} className="bg-[#112544] rounded-lg p-2">
                          <div className="flex items-center justify-between mb-1.5">
                            <span className="text-[11px] font-semibold text-white truncate max-w-[120px]">🏦 {nm}</span>
                            <span className="text-[10px] text-[#A7B8D8]">₺{(totalAmt / 1_000_000).toFixed(1)}M</span>
                          </div>
                          <div className="grid grid-cols-2 gap-1.5">
                            <div
                              className="bg-[#10B981]/10 rounded p-1.5 ring-1 ring-[#10B981]/30 cursor-pointer hover:bg-[#10B981]/20 transition"
                              onClick={() => setNmOpsModal({ nm, opType: 'ikmal', dayKey: cashMgmtSelectedDate, operatorName: operator.name })}
                            >
                              <div className="flex items-center justify-between">
                                <span className="text-[10px] text-[#10B981] font-semibold">💰 İkmal</span>
                                <span className="text-[10px] font-bold text-white">{ops.ikmalCount} adet</span>
                              </div>
                              <div className="text-[11px] font-bold text-[#10B981] mt-0.5">₺{(ops.ikmalAmt / 1000).toLocaleString('tr-TR')}K</div>
                              <div className="text-[9px] text-[#10B981]/60 mt-0.5">▶ ATM listesi</div>
                            </div>
                            <div
                              className="bg-[#F2B705]/10 rounded p-1.5 ring-1 ring-[#F2B705]/30 cursor-pointer hover:bg-[#F2B705]/20 transition"
                              onClick={() => setNmOpsModal({ nm, opType: 'toplama', dayKey: cashMgmtSelectedDate, operatorName: operator.name })}
                            >
                              <div className="flex items-center justify-between">
                                <span className="text-[10px] text-[#F2B705] font-semibold">🚛 Toplama</span>
                                <span className="text-[10px] font-bold text-white">{ops.topCount} adet</span>
                              </div>
                              <div className="text-[11px] font-bold text-[#F2B705] mt-0.5">₺{(ops.topAmt / 1000).toLocaleString('tr-TR')}K</div>
                              <div className="text-[9px] text-[#F2B705]/60 mt-0.5">▶ ATM listesi</div>
                            </div>
                          </div>
                        </div>
                      );
                  ...
                  {/* Kart Toplamı */}
                  {(() => {
                    const totals = operator.nmCenters.reduce((acc, nm) => {
                      const ops = getNmOps(nm, cashMgmtSelectedDate);
                      return {
                        ikmalCount: acc.ikmalCount + ops.ikmalCount,
                        topCount: acc.topCount + ops.topCount,
                        ikmalAmt: acc.ikmalAmt + ops.ikmalAmt,
                        topAmt: acc.topAmt + ops.topAmt,
                      };
                    }, { ikmalCount: 0, topCount: 0, ikmalAmt: 0, topAmt: 0 });
                    return (
                      <div className="mt-2 pt-2 border-t border-[#2B416B]/60 grid grid-cols-2 gap-1.5">
                        <div className="text-center">
                          <div className="text-[10px] text-[#A7B8D8]">Toplam İkmal</div>
                          <div className="text-sm font-bold text-[#10B981]">{totals.ikmalCount} <span className="text-[10px] text-[#A7B8D8]">adet</span></div>
                          <div className="text-[11px] text-[#10B981]">₺{(totals.ikmalAmt / 1_000_000).toFixed(1)}M</div>
                        </div>
                        <div className="text-center">
                          <div className="text-[10px] text-[#A7B8D8]">Toplam Toplama</div>
                          <div className="text-sm font-bold text-[#F2B705]">{totals.topCount} <span className="text-[10px] text-[#A7B8D8]">adet</span></div>
                          <div className="text-[11px] text-[#F2B705]">₺{(totals.topAmt / 1_000_000).toFixed(1)}M</div>
                        </div>
                      </div>
                    );
                  })()}
              ...
              {/* Active coordination + AI Brain insight */}
              ...
              {/* AI brain summary for this operator's NMs */}
              {operator.nmCenters.length > 0 && (() => {
                const nmKey = operator.nmCenters[0].replace(' NM', '').trim();
                const nmData = aiBrainData.nmStats[nmKey];
                const totalCrit = operator.nmCenters.reduce((s, nm) => {
                  const k = nm.replace(' NM', '').trim();
                  return s + (aiBrainData.nmStats[k]?.critical || 0);
                }, 0);
                const totalIkmalAll = operator.nmCenters.reduce((s, nm) => {
                  const k = nm.replace(' NM', '').trim();
                  return s + (aiBrainData.nmStats[k]?.ikmal.length || 0);
                }, 0);
                return totalCrit > 0 ? (
                  <div className="mt-1.5 px-2 py-1 rounded bg-[#EF4444]/10 ring-1 ring-[#EF4444]/30 flex items-center gap-1.5 relative z-10">
                    <span className="text-[10px] animate-pulse">⚠️</span>
                    <span className="text-[10px] text-[#EF4444] font-semibold">{totalCrit} kritik ATM</span>
                    <span className="text-[#A7B8D8]/60 text-[10px]">•</span>
                    <span className="text-[10px] text-[#A7B8D8]">toplam {totalIkmalAll} ikmal kuyruğu</span>
                  </div>
                ) : (
                  <div className="mt-1.5 px-2 py-1 rounded bg-[#10B981]/10 ring-1 ring-[#10B981]/30 flex items-center gap-1.5 relative z-10">
                    <span className="text-[10px]">✅</span>
                    <span className="text-[10px] text-[#10B981] font-semibold">Kritik ATM yok</span>
                    <span className="text-[#A7B8D8]/60 text-[10px]">•</span>
                    <span className="text-[10px] text-[#A7B8D8]">{totalIkmalAll} ikmal planında</span>
                  </div>
                );
              })()}
              </div>
      ...
      {/* NM Operasyon ATM Listesi Modalı */}
      {nmOpsModal && (() => {
        const atmList = getNmAtmList(nmOpsModal.nm, nmOpsModal.opType, nmOpsModal.dayKey);
        const dayLabel = nmOpsModal.dayKey === 'today' ? 'Bugün' : nmOpsModal.dayKey === 'tomorrow' ? 'Yarın' : '+2 Gün';
        const opLabel = nmOpsModal.opType === 'ikmal' ? 'İkmal' : 'Para Toplama';
        const opColor = nmOpsModal.opType === 'ikmal' ? '#10B981' : '#F2B705';
        const opIcon = nmOpsModal.opType === 'ikmal' ? '💰' : '🚛';
        const totalAmt = atmList.reduce((s: number, a: any) => s + a.amount, 0);
        const critCount = atmList.filter((a: any) => a.priority === 'CRITICAL').length;
        const priorityColor: Record<string, string> = { CRITICAL: '#EF4444', HIGH: '#F97316', MEDIUM: '#F2B705', LOW: '#10B981' };
        const priorityLabel: Record<string, string> = { CRITICAL: 'KRİTİK', HIGH: 'YÜKSEK', MEDIUM: 'ORTA', LOW: 'DÜŞÜK' };
        return (
          <div className="fixed inset-0 bg-black/60 flex items-center justify-center p-4" style={{ zIndex: 11000 }}>
            <div className="bg-[#112544] rounded-2xl w-full max-w-2xl max-h-[85vh] flex flex-col ring-2 ring-[#2B416B]">
              {/* Header */}
              <div className="flex items-center justify-between px-5 py-4 border-b border-[#2B416B] bg-[#0E2142]/60 flex-shrink-0">
                <div>
                  <div className="flex items-center gap-2">
                    <span className="text-lg font-bold text-white">{opIcon} {nmOpsModal.nm}</span>
                    <span className="px-2 py-0.5 rounded-full text-xs font-semibold" style={{ background: `${opColor}22`, color: opColor }}>{opLabel}</span>
                    <span className="px-2 py-0.5 rounded-full text-xs bg-[#2E86FF]/20 text-[#2E86FF] font-semibold">{dayLabel}</span>
                    {critCount > 0 && <span className="px-2 py-0.5 rounded-full text-xs bg-[#EF4444]/20 text-[#EF4444] font-bold animate-pulse">{critCount} KRİTİK</span>}
                  </div>
                  <div className="text-xs text-[#A7B8D8] mt-0.5">🧠 AI Kararı • Sorumlu: {nmOpsModal.operatorName} • {atmList.length} ATM</div>
                </div>
                <button onClick={() => setNmOpsModal(null)} className="text-[#A7B8D8] hover:text-white text-2xl">&times;</button>
              </div>

              {/* Summary strip */}
              <div className="grid grid-cols-3 gap-3 px-5 py-3 border-b border-[#2B416B]/60 bg-[#0A1628]/60 flex-shrink-0">
                <div className="text-center">
                  <div className="text-xs text-[#A7B8D8]">ATM Sayısı</div>
                  <div className="text-xl font-bold" style={{ color: opColor }}>{atmList.length}</div>
                </div>
                <div className="text-center">
                  <div className="text-xs text-[#A7B8D8]">Toplam Tutar</div>
                  <div className="text-xl font-bold text-white">₺{(totalAmt / 1_000_000).toFixed(2)}M</div>
                </div>
                <div className="text-center">
                  <div className="text-xs text-[#A7B8D8]">Ort. ATM Başı</div>
                  <div className="text-xl font-bold text-[#2E86FF]">₺{Math.round(totalAmt / Math.max(atmList.length, 1) / 1000).toLocaleString('tr-TR')}K</div>
                </div>
              </div>
              ...
              <div className="flex-1 overflow-y-auto px-5 py-3">
                <div className="space-y-2">
                  {(atmList as any[]).map((atm, idx) => (
                    <div key={atm.atm_id} className="bg-[#0E2142] rounded-xl p-3 ring-1 ring-[#2B416B] hover:ring-[#3B5280] transition">
                      <div className="flex items-start gap-3">
                        <div className="w-8 h-8 rounded-full flex items-center justify-center text-xs font-bold flex-shrink-0 mt-0.5" style={{ background: `${opColor}22`, color: opColor }}>
                          {idx + 1}
                        </div>
                        <div className="flex-1 min-w-0">
                          <div className="flex items-center justify-between">
                            <div className="font-semibold text-sm text-white truncate flex-1 mr-2">{atm.atm_name}</div>
                            <div className="flex items-center gap-1.5 flex-shrink-0">
                              {atm.isSalary && <span className="px-1 py-0.5 rounded text-[9px] bg-[#8B5CF6]/20 text-[#8B5CF6] font-bold">MAAŞ</span>}
                              {atm.priority && <span className="px-1.5 py-0.5 rounded text-[10px] font-bold" style={{ background: `${priorityColor[atm.priority]}22`, color: priorityColor[atm.priority] }}>{priorityLabel[atm.priority]}</span>}
                              <div className="text-sm font-bold" style={{ color: opColor }}>₺{(atm.amount / 1000).toLocaleString('tr-TR')}K</div>
                            </div>
                          </div>
                          <div className="flex items-center gap-2 mt-0.5">
                            <span className="text-[11px] text-[#A7B8D8]">ATM {atm.atm_id}</span>
                            <span className="text-[#A7B8D8]/40">•</span>
                            <span className="text-[11px] text-[#A7B8D8]">{atm.city} / {atm.district}</span>
                            {atm.location_type && <span className="px-1.5 py-0.5 rounded text-[10px] bg-[#2B416B]/60 text-[#A7B8D8]">{atm.location_type}</span>}
                            {atm.zone && <span className="text-[10px] text-[#A7B8D8]/60">Z{atm.zone}</span>}
                          </div>
                          {atm.cashLevel !== undefined && (
                            <div className="mt-1.5">
                              <div className="flex items-center justify-between mb-0.5">
                                <span className="text-[10px] text-[#A7B8D8]">Nakit Seviyesi</span>
                                <span className="text-[10px] font-bold" style={{ color: atm.cashLevel < 20 ? '#EF4444' : atm.cashLevel < 40 ? '#F2B705' : '#10B981' }}>{atm.cashLevel}%</span>
                              </div>
                              <div className="w-full h-1.5 rounded-full bg-[#2B416B]/60">
                                <div
                                  className="h-1.5 rounded-full transition-all"
                                  style={{
                                    width: `${atm.cashLevel}%`,
                                    background: atm.cashLevel < 20 ? '#EF4444' : atm.cashLevel < 40 ? '#F2B705' : '#10B981'
                                  }}
                                />
                              </div>
                            </div>
              ...
              {/* Footer */}
              <div className="px-5 py-3 border-t border-[#2B416B] bg-[#0E2142]/60 flex items-center justify-between flex-shrink-0">
                <div className="text-xs text-[#A7B8D8]">₺ Toplam: <span className="font-bold text-white">{totalAmt.toLocaleString('tr-TR')}</span></div>
                <button onClick={() => setNmOpsModal(null)} className="px-4 py-2 rounded-lg bg-[#2E86FF] hover:bg-[#0066FF] text-white text-sm font-semibold transition">Kapat</button>
              </div>
            </div>
          </div>
        );
